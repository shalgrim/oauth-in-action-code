...
app.get("/", function(req, res) {
  res.render("index", { access_token: access_token, scope: scope }); // res.render is an epxressJS thing
});
...
app.get("/callback", function(req, res) {
  ...
  if (req.query.error) {
    res.redirect("error", { error: req.query.error });
    return;
  }
  ...
  if (tokRes.statusCode >= 200 && tokRes.statusCode < 300) {
    ...
    res.render("error", {
      error:
        "Unable to fetch access token, server response: " + tokRes.statusCode // Why do we use interpolation sometimes and concatenation others?
    });
  }
});
