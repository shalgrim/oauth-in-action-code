...
app.post("/approve", function(req, res) {
  ...
  if (req.body.approve) {
    ...
    var urlParsed = buildUrl(query.redirectu_uri, { error: "access_denied" });
    res.redirect(urlParsed);
    return;
  }
});
...
app.post("/token", function(req, res) {
  ...
  if (req.body.client_id) {
    // second way to send credentials
    if (clientId) {
      // if they sent both ways that's an error
      res.status(401).json({ error: "invalid_cilent" });
      return;
    }
